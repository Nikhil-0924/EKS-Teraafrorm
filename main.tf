...
resource "aws_subnet" "nikhil_subnet" {
  count = 2
...
resource "aws_route_table_association" "a" {
  count          = 2
  subnet_id      = aws_subnet.nikhil_subnet[count.index].id
  route_table_id = aws_route_table.nikhil_route_table.id
}
...
resource "aws_eks_node_group" "nikhil" {
  cluster_name    = aws_eks_cluster.nikhil.name
...
resource "aws_iam_role_policy_attachment" "nikhil_cluster_role_policy" {
  role       = aws_iam_role.nikhil_cluster_role.name
...
resource "aws_iam_role_policy_attachment" "nikhil_node_group_role_policy" {
  role       = aws_iam_role.nikhil_node_group_role.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKSWorkerNodePolicy"
}

resource "aws_iam_role_policy_attachment" "nikhil_node_group_cni_policy" {
  role       = aws_iam_role.nikhil_node_group_role.name
